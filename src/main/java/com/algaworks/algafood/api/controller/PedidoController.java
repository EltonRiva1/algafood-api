...
public class PedidoController {
	...
	public PedidoModel adicionar(@Valid @RequestBody PedidoInput pedidoInput) {
		try {
			Pedido novoPedido = this.pedidoInputDisassembler.toDomainObject(pedidoInput);
			novoPedido.setCliente(new Usuario());
			novoPedido.getCliente().setId(1L);
			novoPedido = this.emissaoPedidoService.emitir(novoPedido);
			return this.pedidoModelAssembler.toModel(novoPedido);
